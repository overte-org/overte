...
//  placesHtml.js
//
//  Created by Alezia Kurdis, August 30th, 2025.
//  Copyright 2025 Overte e.V.
//
//  javascript for the HTML UI of the Places App.
//
...
var locationBookmarks = {};
var bookmarkEditMode = false;
...
EventBridge.scriptEventReceived.connect(function (message) {
    ...
    if (message.channel === channel && message.action === "BOOKMARKS_DATA") {
        locationBookmarks = message.data;
        if (currentListFormat === "BOOKMARKS") {
            generateContent();
        }
    }
});
...
function toggleBookmarkEditMode() {
    if (bookmarkEditMode) {
        ...
        document.getElementById("bookmarkEditMode").className = "bookmarkActionOff";
    } else {
        bookmarkEditMode = true;
        document.getElementById("bookmarkEditMode").className = "bookmarkActionOn";
    }
    generateContent();
}
...
function deleteBookmark(name) {
    ...
    EventBridge.emitWebEvent(JSON.stringify(bookmarkOrder));
}

function renameBookmark(name, url) {
    document.getElementById("originalName").value = name;
    document.getElementById("nameRenameBookmark").value = name;
    document.getElementById("bookmarkUrl").innerHTML = escapeHTML(url);
    document.getElementById("urlRenameBookmark").value = url;
    document.getElementById("bookmarkEditorAddUi").style.display = "none";
    document.getElementById("bookmarkEditorRenameUi").style.display = "block";
    document.getElementById("bookmarkNameEditor").style.transform = "translateX(0%)";
}

function requestBookmarkToRename() {
    let originalName = document.getElementById("originalName").value;
    let name = document.getElementById("nameRenameBookmark").value;
    let url = document.getElementById("urlRenameBookmark").value;
...
function addBookmark() {
    document.getElementById("nameAddBookmark").value = getPossiblePlaceName(currentLocation);
    document.getElementById("bookmarkEditorAddUi").style.display = "block";
    document.getElementById("bookmarkEditorRenameUi").style.display = "none";
    document.getElementById("bookmarkNameEditor").style.transform = "translateX(0%)";
}

function requestBookmarkToAdd() {
    let name = document.getElementById("nameAddBookmark").value;
    if (name === "") { 
        return; 
    } else {
        let bookmarkOrder = {
...
function cancelBookMarkAction() {
    document.getElementById("bookmarkNameEditor").style.transform = "translateX(-100%)";
}

function getPossiblePlaceName(url) {
    if (url.slice(0, 5).toLowerCase() === "hifi:") {
        return url.split("/")[2];
    } else {
        return url.match(/([^\/]+)\.json(?:\?.+)?$/)[1];
    }
}
...
function generateContent() {
    ...
    document.getElementById("placesFiltersBar").style.display = "none";
    document.getElementById("bookmarkToolBar").style.display = "none";
    ...
    } else if (currentListFormat === "BOOKMARKS") { 
        // ########## BOOKMARK ############
        var formattedBookmarkList = "";
        var bookmarksCounter = 0;
        var totalBookmarksCounter = 0;
    
        document.getElementById("searchFiltersBar").style.display = "none";
        document.getElementById("bookmarkToolBar").style.display = "inline-block";

        for (const [name, url] of Object.entries(locationBookmarks)) {
            let lowerCasename = name.toLowerCase();
            if (lowerCasename.indexOf(searchedText.toLowerCase()) !== -1) {
            
                let bookmarkType = "Unknown";
                if (url.slice(0, 5).toLowerCase() === "hifi:") {
                    bookmarkType = "Domain";
                } else if ((url.split("?")[0]).slice(-5).toLowerCase() === ".json") {
                    bookmarkType = "Serverless";
                }
                
                formattedBookmarkList += "<div class='bookmarkEntry" + bookmarkType + "' id='bookmark-" + bookmarksCounter + "'>";
                formattedBookmarkList += "<table style = 'width: 100%; height: 100%; border-collapse: collapse;'>";
                formattedBookmarkList += "<tr><td class='bookmarkName'>";
                formattedBookmarkList += "<span class='bookmark' onclick='teleport(" + '""' + ", " + escapeStringHTML(url) + "); return false;'>" + truncateString(escapeHTML(name), 37) + "</span>";
                formattedBookmarkList += "</td><td class='bookmarkActions'>";
                if (bookmarkEditMode) {
                    formattedBookmarkList += "<a class='bookmarkInlineAction' onClick='renameBookmark(" + escapeStringHTML(name) + ", " + escapeStringHTML(url) + "); return false;'>s</a>";
                    formattedBookmarkList += "&nbsp;";
                    formattedBookmarkList += "<a class='bookmarkInlineDelete' onClick='deleteBookmark(" + escapeStringHTML(name) + "); return false;'>w</a>";
                } else {
                    formattedBookmarkList += "<a class='bookmarkInlineAction' onClick='copyPlaceURL(" + escapeStringHTML(url) + "); return false;'>.</a>";
                    formattedBookmarkList += "&nbsp;";
                    formattedBookmarkList += "<a class='bookmarkInlineAction' onClick='rezPortal(" + escapeStringHTML(name) + ", " + escapeStringHTML(url) + ", " + escapeStringHTML(name + url) + "); return false;'>/</a>";
                }
                formattedBookmarkList += "</td></tr>";
                formattedBookmarkList += "</table>";
                formattedBookmarkList += "</div>";

                bookmarksCounter++;
            }
            totalBookmarksCounter++;
        }

        document.getElementById("list_content").innerHTML = formattedBookmarkList;
        
        var bookmarksPageInfo = " bookmarks";
        if (bookmarksCounter <= 1) {
            bookmarksPageInfo = " bookmark";
        }
        document.getElementById("listInfo").innerHTML = bookmarksCounter + bookmarksPageInfo + " of " + totalBookmarksCounter + ".";
        // ########## END BOOKMARK ############
        
    } else {
        document.getElementById("placesFiltersBar").style.display = "inline-block";
        ...
        for (var i = 0; i < placeRecords.length; i++) {
            ...
            This is the data that you can use to build each portal:
                ...
                placeRecords[i].accessStatus
                    ...
                    //This indicates if there are presence of people or not, and if the domain is at full capacity.
                ...
                placeRecords[i].domainAccessStatus
                    ...
                    //This indicates if there are presence of people or not, and if the domain is at full capacity.
                    //This will be used to display "Life" indicator (green) or "Full" (red)
                    //But this one is overridden in case there is a place specific count to 0.
                ...
                placeRecords[i].place_attendance
                    Number (The number of people currently in this specific Place in the domain. Default -1, available only if the place is in the domain where the user is currently connected to. In that case, it must be 0 or greater)
...
function displayTab(tab) {
    ...
    const buttons = {
        "PLACES": "listFormatPlaces",
        "DOMAINS": "listFormatDomains",
        "REGIONS": "listFormatMetaverse",
        "BOOKMARKS": "listFormatBookmarks"
    };
    ...
    if (buttons[tab]) {
        document.getElementById(buttons[tab]).disabled = true;
    }
...
function openDetail(placeId) {
    ...
    document.getElementById("placeDetail").style.transform = "translateX(0%)";
}

function closeDetail() {
    document.getElementById("placeDetail").style.transform = "translateX(-100%)";
}
