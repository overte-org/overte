...
void UdtSocket_receive::ingestData(DataPacket&& dataPacket, const QElapsedTimer& timeReceived) {
    ...
    if (((static_cast<quint32>(packetID) - 1) & 0xf) == 0) {
        if (_recvLastProbe.isValid()) {
            _recvPktPairHistory.append(
                std::chrono::microseconds((timeReceived.nsecsElapsed() - _recvLastProbe.nsecsElapsed()) / 1000));
    ...
    if (_recvLastArrival.isValid()) {
        _recvPktHistory.append(
            std::chrono::microseconds((timeReceived.nsecsElapsed() - _recvLastArrival.nsecsElapsed()) / 1000));
