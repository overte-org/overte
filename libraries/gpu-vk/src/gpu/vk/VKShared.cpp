...
VkFormat gpu::vk::evalTexelFormatInternal(const gpu::Element& dstFormat) {
    ...
    switch (dstFormat.getDimension()) {
        case gpu::SCALAR:
        {
            switch (dstFormat.getSemantic()) {
                ...
                case gpu::DEPTH:
                    ...
                    switch (dstFormat.getType()) {
                        // Vulkan doesn't support any other depth formats
                        case gpu::UINT32:
                        case gpu::INT32:
                        case gpu::NUINT32:
                        case gpu::NINT32:
        ...
        {
            switch (dstFormat.getSemantic()) {
                    // VKTODO: Handle more component types
                case gpu::RGB:
                case gpu::RGBA:
                    switch (dstFormat.getType()) {
                        case gpu::UINT8:
        ...
        case gpu::VEC4:
        {
            switch (dstFormat.getSemantic()) {
                ...
                case gpu::SRGB:
                    ...
                    // VKTODO: SRGB originally was VK_FORMAT_R8G8B8_SRGB, but then number of bytes doesn't match.
                    break;
                case gpu::SRGBA:
                    switch (dstFormat.getType()) {
                        case gpu::NUINT8:
