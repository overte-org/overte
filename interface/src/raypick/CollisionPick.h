...
protected:
    ...
    void computeShapeInfo(const CollisionRegion& pick, ShapeInfo& shapeInfo, std::shared_ptr<GeometryResource> resource);
    void computeShapeInfoDimensionsOnly(const CollisionRegion& pick, ShapeInfo& shapeInfo, std::shared_ptr<GeometryResource> resource);
    void filterIntersections(std::vector<ContactTestResult>& intersections) const;

    bool _scaleWithParent;

    PhysicsEnginePointer _physicsEngine;
    std::shared_ptr<GeometryResource> _cachedResource;
