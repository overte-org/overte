...
void Application::updateCamera(RenderArgs& renderArgs, float deltaTime) {
    ...
    if (mode == CAMERA_MODE_FIRST_PERSON || mode == CAMERA_MODE_FIRST_PERSON_LOOK_AT) {
        ...
        if (isHMDMode()) {
            ...
            if (getCameraClippingEnabled()) {
                auto result = DependencyManager::get<PickManager>()->getPrevPickResultTyped<RayPickResult>(_cameraClippingRayPickID);
    ...
    const bool shouldEnableCameraClipping = (mode == CAMERA_MODE_THIRD_PERSON || mode == CAMERA_MODE_LOOK_AT || mode == CAMERA_MODE_SELFIE) &&
        !isHMDMode() && getCameraClippingEnabled();
