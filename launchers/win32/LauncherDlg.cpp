...
static CString RALEYWAY_REGULAR = _T("Raleway-Regular");
static CString RALEYWAY_SEMIBOLD = _T("Raleway-SemiBold");
...
BOOL CLauncherDlg::OnInitDialog() {
    ...
    CFont editFont;
    if (LauncherUtils::getFont(RALEYWAY_REGULAR, FIELDS_FONT_SIZE, true, editFont)) {
        m_orgname.SetFont(&editFont);
        m_username.SetFont(&editFont);
        m_password.SetFont(&editFont);
    }
    CFont buttonFont;
    if (LauncherUtils::getFont(_T("Raleway-Bold"), BUTTON_FONT_SIZE, true, buttonFont)) {
...
HBRUSH CLauncherDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
    ...
    if (getTextFormat(resId, textFormat)) {
        ...
        CFont textFont;
        CString fontFamily = textFormat.isBold ? RALEYWAY_SEMIBOLD : RALEYWAY_REGULAR;
...
void CLauncherDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    if (nIDCtl == IDC_BUTTON_NEXT) {
        ...
        CFont buttonFont;
        if (LauncherUtils::getFont(RALEYWAY_SEMIBOLD, BUTTON_FONT_SIZE, true, buttonFont)) {
        ...
        CFont buttonFont;
        if (LauncherUtils::getFont(RALEYWAY_SEMIBOLD, TROUBLE_FONT_SIZE, true, buttonFont)) {
            dc.SelectObject(buttonFont);
        }
        ...
        CFont buttonFont;
        if (LauncherUtils::getFont(RALEYWAY_SEMIBOLD, TERMS_FONT_SIZE, true, buttonFont)) {
            dc.SelectObject(buttonFont);
        }
